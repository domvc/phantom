...
// By default we don't show past weeks — the calendar is forward-looking.
// "See recent workouts" toggle reveals the last RECENT_WEEKS_BACK weeks on demand.
const RECENT_WEEKS_BACK = 2;
const WEEKS_FWD = 4; // current + 4 weeks ahead
...
export default function CalendarPage() {
  ...
  // Build the visible week list: current + future, plus optional recent weeks
  const weeks: { monday: Date; isCurrent: boolean; phase: PlanPhase | null }[] = [];
  const startOffset = showRecent ? -RECENT_WEEKS_BACK : 0;
  for (let i = startOffset; i <= WEEKS_FWD; i++) {
...
function ActualSessionCard({ reconciliation: r }: { reconciliation: SessionReconciliation }) {
  ...
  return (
    <div
      className={`rounded-md border px-2.5 py-2 min-h-[88px] flex flex-col ${style.bg} ${style.border}`}
      title={r.message}
    >
      <div className="flex items-center gap-1.5 mb-0.5">
        <span
